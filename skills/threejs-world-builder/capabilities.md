...
| The place around the stage: a town, a valley, fields, a river, roads, farms, people going about | `planSettlement` → `createTerrain` (relief, rivers, roads, pads, smoothing) → `buildSettlement` + `buildBridges` → `scatterVegetation` + `scatterProps` + `populate`, then `setupOutdoorRendering({scale:'large'})`; everything seeded and instanced; the stage sits on the terrain pad at y=0; expose `worldTest.groundHeight` |
...
| Hand props, tools, furniture, set dressing | copy `parts.js` + `props.js` (compose anything missing from the parts patterns, seeded)|
...
| People or crowds | `parts.jointedFigure`: one skinned body with fabric-textured clothes. Give each person what the brief implies: `body:'f'`, `child:true`, `dress:{color,pattern}`, `coat`, `apron`, `shorts`, `topPattern` (stripes/check/dots/floral), `collar`, `hat:{kind}`, and a different `seed` each so faces, hair and fabrics differ. Move it with `characters.walkBetween`/`walkAlong` or pass `distance` to `pose` so the feet plant|
| Animals | `parts.quadruped` (dog, horse; `shoulder` scales it to cat/sheep/cow) and `parts.fowl` (hen, rooster)|
| Vehicles, carts, boats | `createHandCart`, `createBoat` / `loftedHull` + `spokedWheel` in parts.js|
